...
def ResNet50(include_top=True, weights='imagenet',
    ...
    x = ZeroPadding2D((3, 3), trainable=trainable)(img_input)
    x = Conv2D(8, (7, 7), strides=(2, 2), name='conv1', trainable=trainable)(x)
    ...
    x = identity_block(x, 3, [16, 16, 64], stage=3, block='c', trainable=trainable)
    # x = identity_block(x, 3, [128, 128, 512], stage=3, block='d', trainable=trainable)
    #
    x = conv_block(x, 3, [32, 32, 128], stage=4, block='a', trainable=trainable)
    x = identity_block(x, 3, [32, 32, 128], stage=4, block='b', trainable=trainable)
    x = identity_block(x, 3, [32, 32, 128], stage=4, block='c', trainable=trainable)
    # x = identity_block(x, 3, [256, 256, 1024], stage=4, block='d', trainable=trainable)
    # x = identity_block(x, 3, [256, 256, 1024], stage=4, block='e', trainable=trainable)
    # x = identity_block(x, 3, [256, 256, 1024], stage=4, block='f', trainable=trainable)
    #
    # x = conv_block(x, 3, [512, 512, 2048], stage=5, block='a', trainable=False)
    # x = identity_block(x, 3, [512, 512, 2048], stage=5, block='b', trainable=False)
    # x = identity_block(x, 3, [512, 512, 2048], stage=5, block='c', trainable=False)
